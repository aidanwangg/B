...
def _block_to_squares(board: Block) -> (
    ...
    if board.colour is not None:
        ...
        squares = []
        for i in range(0, 4):
            squares.extend(_block_to_squares(board.children[i]))
    return squares
...
class Block:
    ...
    def smash(self) -> bool:
        ...
        if not self.smashable():
            ...
            for i in range(0, 4):
                child = Block(child_pos[i], self.child_size(), None,
                              self.level + 1, self.max_depth)

                rdm_value = random.random()
                if (rdm_value < math.exp(
                        -0.25 * self.level) and child.smashable()):
                    child.smash()
                else:
    ...
    def combine(self) -> bool:
        ...
        for child in self.children:
            if child.colour is None:
                return False
            elif child.colour not in colours:
        ...
        for colour in colours:
            if colours[colour] >= 3:
                big_colour = colour
            elif colours[colour] == 2 and not twice:
                twice = True
                big_colour = colour
            elif colours[colour] == 2:
                return False

        if big_colour:
            self.colour = big_colour
            self.children = []
            return True
        return False
    ...
    def create_copy(self) -> Block:
        ...
        True
        """
        new = Block(self.position, self.size, self.colour,
                    self.level, self.max_depth)
        if self.colour is not None:
            return new
        else:
            for child in self.children:
                new.children.append(child.create_copy())
            return new
